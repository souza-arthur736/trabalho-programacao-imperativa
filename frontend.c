...
#define ARQ_PERFIS "pacientes_cadastrados.csv"
...
typedef struct {
    ...
    char nome[120];
    int idade;
    char cpf[40], telefone[60], email[120], endereco[180], dataNascimento[50], observacoes[400];
} PerfilPaciente;

typedef struct {
    GtkApplication *app;
    GtkWidget *janela, *listaPacientes, *area;
    Dentista dentista;
    PerfilPaciente pacienteAtual;
    int temPacienteAtual;
    int linhaDiagEdicao;

    GtkWidget *nome, *idade, *cpf, *telefone, *email, *endereco, *dataNascimento, *observacoes;
    GtkWidget *dataExame, *maxila, *mmMaxila, *perfil, *coa, *cogn, *afai, *anb, *sngogn, *dist1na, *ang1na, *dist1nb, *ang1nb;
    GtkWidget *resultado, *status;
    char ultimoLaudo[TAM_DIAGNOSTICO];
    char ultimaClasse[80], ultimaMaxila[80], ultimaMandibula[80], ultimoCrescimento[80], ultimoPerfil[80];
} Painel;
